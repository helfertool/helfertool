...
class BadgeDefaultsForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["role"].queryset = BadgeRole.objects.filter(badge_settings=self.settings.pk)
        self.fields["design"].queryset = BadgeDesign.objects.filter(badge_settings=self.settings.pk)
...
class BadgeJobDefaultsForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        roles = BadgeRole.objects.filter(badge_settings=self.event.badge_settings.pk)
        designs = BadgeDesign.objects.filter(badge_settings=self.event.badge_settings.pk)
    ...
    def save(self):
        for job in self.event.job_set.all():
            job.badge_defaults.design = self.cleaned_data["job_%d_design" % job.pk]
            job.badge_defaults.role = self.cleaned_data["job_%d_role" % job.pk]
            job.badge_defaults.no_default_role = self.cleaned_data["job_%d_no_def_role" % job.pk]
